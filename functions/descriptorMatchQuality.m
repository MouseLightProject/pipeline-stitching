...
for t = latticeZRange(1:end-1)'
    ...
    for id_ix = find(ix)
        ...
        layer{cnt} = regpts{id_ix}.X;  %#ok<AGROW>
        %layerp1{cnt} = regpts{id_ix}.Y;  %#ok<AGROW>
        % apply tforms
        ...
        poslayer_t{cnt} = Layer_t;  %#ok<AGROW>
        %poslayer_tp1{cnt} = Layer_tp1;  %#ok<AGROW>
    end
    %%
    if isempty(layer)
        continue
    ...
    XYZ_t{t} = cat(1,poslayer_t{:});  %#ok<AGROW>
...
frame_count = length(latticeZRange)-1 ;
F(frame_count) = struct('cdata',[], 'colormap',[]) ;
for frame_index = 1 : frame_count  ,
    t = latticeZRange(frame_index) ;
    %%
%     idxtest = sliceinds(375)
    ...
    myaxes = axes('Parent', myfig, ...
                  ...
                  'Color',[1 1 1]*.8) ;
    %hold(myaxes, 'on') ;
    if t <= size(XYZ_t,2) && ~isempty(XYZ_t{t}) ,
        XYZ_t_this = XYZ_t{t} ;
        %X = XYZ_t_this(:,1);
        %Y = XYZ_t_this(:,2);
        %Z = XYZ_t_this(:,3);
...
%             'ZData', XYZ_t_this(:,3), ...
        ...
        for ii=1:sum(ix)
            rectangle(myaxes, 'Position', [x(ii) y(ii) w(ii) h(ii)]) ;
        ...
        title(myaxes, [num2str(t),' - ', num2str(max(D(ix)))]) ;
    end
